...
def upload_file():
    if request.method == 'GET':
        ...
        try:
            ...
            file.save(full_name)
            img= image.load_img(full_name, color_mode="grayscale", target_size=(258,540,1))
            ...
            print(result.shape)
            k=file.filename
            d_full_name = os.path.join(UPLOAD_FOLDER, k[0:-4]+".jpg")
            plt.imsave(d_full_name,result.reshape(258, 540), cmap="gray")
            #predicted_class = np.asscalar(np.argmax(result, axis=1))
            #accuracy = round(result[0][predicted_class] * 100, 2)
            #label = indices[predicted_class]
            print(d_full_name)
            return render_template('predict.html', image_file_name=d_full_name)
        except:
            flash("There could be many reasons for errors such as image format is not right, image is not uploaded etc., so please have a check and then try again", "danger")      
